...
<div class="content" data-ice="content"><div data-ice="index" class="github-markdown"><h1 id="kkbox-open-api-developer-sdk-for-javascript">KKBOX Open API Developer SDK for JavaScript</h1>
<p>The SDK for accessing various metadata of KKBOX tracks, albums, artists, playlists and stations.</p>
<h3 id="npm-install">npm install</h3>
<pre><code><code class="source-code prettyprint">npm install @kkboxorg/kkbox-javascript-developer-sdk</code>
</code></pre><h3 id="source-install">Source Install</h3>
<pre><code><code class="source-code prettyprint">npm install</code>
</code></pre><h3 id="build-sdk">Build SDK</h3>
<pre><code><code class="source-code prettyprint">npm run build</code>
</code></pre><h3 id="test">Test</h3>
<p>For testing, you should first browse <a href="https://developer.kkbox.com/">https://developer.kkbox.com/</a> and create an developer account, then create an app to get client id and client secret for that account.</p>
<p>Then, create a file name <code>client_secrets.json</code>, put it in the <code>test</code> directory and write the client id and client secret in it. The content will be like:</p>
...
</code></pre><p>And then we can run the tests.</p>
<pre><code><code class="source-code prettyprint">npm run test</code>
</code></pre><h3 id="sdk-documentation">SDK Documentation</h3>
<p>Please browse <a href="https://kkbox.github.io/OpenAPI-JavaScript/">https://kkbox.github.io/OpenAPI-JavaScript/</a></p>
<h2 id="how-to-use-the-sdk">How to use the SDK</h2>
<p>There are two classes Auth and Api and you should initialize an Auth object by client id and secret.</p>
<pre><code><code class="source-code prettyprint">import {Auth} from &apos;@kkboxorg/kkbox-javascript-developer-sdk&apos;

const auth = new Auth(client_id, client_secret)</code>
</code></pre><p>Then use the auth object to get access token.</p>
<pre><code><code class="source-code prettyprint">auth.clientCredentialsFlow.fetchAccessToken().then(response =&gt; {
    const access_token = response.data.access_token
})</code>
</code></pre><p>After getting access token, use it to initialize Api object.</p>
<pre><code><code class="source-code prettyprint">import {Api} from &apos;@kkboxorg/kkbox-javascript-developer-sdk&apos;

const api = new Api(access_token)</code>
</code></pre><p>Now you can use various fetcher object to fetch data.</p>
<pre><code><code class="source-code prettyprint">api.searchFetcher.setSearchCriteria(&apos;&#x4E94;&#x6708;&#x5929; &#x6D3E;&#x5C0D;&#x52D5;&#x7269;&apos;, &apos;track&apos;).fetchSearchResult().then(response =&gt; {
    console.log(response.data)
})</code>
</code></pre><p>Most methods return paged result and we can use the <code>fetchNextPage</code> method to get the next page of result.</p>
<pre><code><code class="source-code prettyprint">api.searchFetcher.setSearchCriteria(&apos;&#x4E94;&#x6708;&#x5929; &#x6D3E;&#x5C0D;&#x52D5;&#x7269;&apos;, &apos;track&apos;).fetchSearchResult().then(response =&gt; {
    console.log(response.data)
...
</code></pre><p>Then open the the file <code>public/index.html</code></p>
